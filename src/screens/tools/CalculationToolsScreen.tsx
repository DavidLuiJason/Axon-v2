  ...
  const inputPercent = () => {
    ...
    if (prevValue !== null && operator) {
      if (operator === '+' || operator === '-') {
        // Contextual percentage of previous value (e.g. 100 + 15% -> 15)
        const percentVal = (prevValue * val) / 100;
        setDisplayValue(String(parseFloat(percentVal.toFixed(8))));
      } else if (operator === '×' || operator === '÷') {
        // Factor percentage (e.g. 200 × 5% -> 0.05)
        const percentVal = val / 100;
        setDisplayValue(String(parseFloat(percentVal.toFixed(8))));
      }
    } else {
      // Standalone unary percentage
      setDisplayValue(String(parseFloat((val / 100).toFixed(8))));
    }
  };
