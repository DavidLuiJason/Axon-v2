...
interface AppContextType {
  ...
  addMessage: (text: string, attachment?: { name: string; type: string; size?: string; dataUrl?: string }) => void;
  ...
  exportConversationToFile: (format: 'markdown' | 'text' | 'json') => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addMessage = async (
    ...
    // 1. Check for manual account switch command (e.g. "log into account B", "switch to account B")
    ...
    // 2. Check for AXON self-knowledge query (e.g. "who are you", "what can you do", "describe yourself")
  ...
  const extractSingleMessageToNote = (message: ChatMessage, targetProjectId?: string): NoteItem => {
    ...
    const preview = message.text.substring(0, 35).replace(/\n/g, ' ');
