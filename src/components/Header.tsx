...
export const Header: React.FC<HeaderProps> = ({ onOpenMenu }) => {
  ...
  return (
    <header
      id="app-header"
      className="sticky top-0 z-40 h-14 bg-black/90 backdrop-blur-md border-b border-neutral-800/80 px-3 flex items-center justify-between select-none"
    >
      ...
      <div className="flex items-center gap-1.5">
        ...
        {isMainScreen && (
          ...
          >
            ...
            {/* Split (Halfway) */}
            <button
              id="pane-view-split-btn"
              type="button"
              onClick={() => setPaneViewState('split')}
              title="Halfway Split: Side-by-Side"
              className={`p-1.5 rounded-lg transition-all ${
                paneViewState === 'split'
                  ? 'bg-white text-black shadow-sm'
                  : 'text-neutral-400 hover:text-white'
            ...
            <button
              ...
              title="Full Right: Workspace Only"
              className={`p-1.5 rounded-lg transition-all ${
